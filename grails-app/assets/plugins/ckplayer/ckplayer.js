	...
	ckplayer.prototype = {
		...
		*/
		addButtonEvent: function() {
			...
			this.progressClick(volumeClickObj,'volume');
		...
		*/
		progressClick: function(obj,volume) {
			/*
				refer:参考对象
				fun:返回函数
				refer:参考元素，即背景
			...
			*/
			...
			var referMouseClick = function(event) {
				...
				if (obj['fun']) {
					...
					debugger
					if (volume !== 'volume'){
						if (nowZ >= jinduTime && listplayer <= trial[0].video.length - 1){
							if (listplayer !== trial[0].video.length - 1){
								while (nowZ >= (jinduTime + trial[0].video[listplayer].videolength)){
									jinduTime = jinduTime + trial[0].video[listplayer].videolength;
									listplayer++;
									if (listplayer == trial[0].video.length - 1){
										/*var seektime1 = nowZ-jinduTime
                                        player.newVideo({
                                            autoplay: true,
                                            video: videoarr[listplayer].url,
                                            forceduration: totaltime, //强制使用为播放总时间
                                            seek: seektime1
                                        })*/
										break
									}
							...
							player.newVideo({
								...
								video: videoarr[listplayer].url,
								forceduration: totaltime, //强制使用为播放总时间
								volume: thisTemp.V.volume,
								seek: seektime1
							})
							// seektime = seektime + trial[0].video[listplayer].videolength;

						}else {//判断小于分段开始视频时间的时候

							while (nowZ <= jinduTime){
								jinduTime = jinduTime - trial[0].video[listplayer-1].videolength;
								listplayer--;
								/*if (jinduTime >= trial[0].video.length - 1){
                                    var seektime1 = nowZ-jinduTime
                                    player.newVideo({
                                        autoplay: true,
                                        video: videoarr[listplayer].url,
		...
		/*intoVideo: function(url,listplayer1){//处理播放通道地址
			var thisTemp = this;
			//需要跳转的时间
			jinduTime = 0;
			for (var i=0; i < listplayer1;i++){
				jinduTime = jinduTime + trial[0].video[i].videolength;
			}
			listplayer = parseInt(listplayer1);
			//切换地址
			player.newVideo({
				autoplay: true,
				video: url,
		...
		*/
		timeUpdateHandler: function() {
			...
			if (duration > 0) {
				...
				// this.jinduTime = this.V.jinduTime;
				this.timeTextHandler();
		...
		*/
		timeTextHandler: function() { //显示时间/总时间
			...
			// var time =this.currentTime + this.V.jinduTime;
			var time = this.V.currentTime + jinduTime;
			if (isNaN(duration) || parseInt(duration) < 0.2) {
				duration = this.vars['duration'];
			}
			if(this.vars['forceduration']>0){
